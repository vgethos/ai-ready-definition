...
export default function PatternFlipCard() {
  const [isPrefill, setIsPrefill] = useState(true);
  const [isComplete, setIsComplete] = useState(false);

  // Card state
  const [cards, setCards] = useState<FlipCard[]>(() => buildInitialCards(true));
  ...
  function buildInitialCards(prefill: boolean): FlipCard[] {
    return [
      {
        id: "spouse",
        category: "spouse",
        label: "My spouse\nor partner",
        householdName: prefill ? HOUSEHOLD_MEMBERS[0] : undefined,
        name: prefill ? HOUSEHOLD_MEMBERS[0] : "",
        birthdate: "",
        isComplete: false,
        isPrimary: true,
      },
      {
        ...
        category: "children",
        label: "One of my\nadult children",
        householdName: prefill ? HOUSEHOLD_MEMBERS[1] : undefined,
        name: prefill ? HOUSEHOLD_MEMBERS[1] : "",
        birthdate: "",
        isComplete: false,
        isPrimary: false,
      },
  ...
  const resetState = () => {
    setIsComplete(false);
    setCards(buildInitialCards(isPrefill));
  ...
  const handleTogglePrefill = () => {
    const next = !isPrefill;
    setIsPrefill(next);
    setIsComplete(false);
    setCards(buildInitialCards(next));
    setFocusedCardId(null);
    setIsFlipped(false);
    setAnimPhase("idle");
    setSlideDirection("left");
  ...
  const saveCard = (id: string) => {
    setCards((prev) =>
      prev.map((card) => {
        if (card.id === id) {
          const complete = card.name.trim() !== "" && card.birthdate.trim() !== "";
          return { ...card, isComplete: complete };
        }
        return card;
      })
    );
  ...
  // Close: flip back, then shrink
  const closeCard = () => {
    setIsFlipped(false);
    setTimeout(() => setFocusedCardId(null), 500);
  };

  const updateCard = (id: string, updates: Partial<FlipCard>) => {
    setCards((prev) =>
  ...
  // ──── CARD FRONT (thumbnail in grid) ────
  const renderCardFront = (card: FlipCard) => {
    return (
      <button
        key={card.id}
        onClick={() => {
          if (!focusedCardId) openCard(card.id);
        }}
        className="w-full bg-white rounded-2xl border border-gray-20 hover:border-gray-40 transition-all cursor-pointer flex flex-col items-center justify-center text-center relative"
        style={{ padding: card.isPrimary ? "28px 20px" : "24px 12px" }}
      >
        {/* Completion checkmark */}
        {card.isComplete && (
          <div className="absolute top-3 right-3 w-[22px] h-[22px] rounded-full bg-cypress-100 flex items-center justify-center">
  ...
  // ──── ENLARGED FLIP OVERLAY ────
  const renderFocusedCard = () => {
        ...
        >
          <div
            ...
            style={{
              ...
              pointerEvents: "auto",
              // Zoom-in animation
              animation: "cardZoomIn 0.35s ease-out forwards",
            }}
          >
            <div
            ...
            >
              {/* FRONT — enlarged version */}
              <div
                className="w-full rounded-2xl border border-gray-20 bg-white"
                style={{ backfaceVisibility: "hidden" }}
              >
                <div className="flex flex-col items-center justify-center text-center py-10 px-6">
                  <div className="mb-4">
                    {CARD_ICON[focusedCard.category]}
                  </div>
                  <div className="font-sans text-[18px] text-gray-100 leading-snug whitespace-pre-line">
                    {focusedCard.label}
                  </div>
                  {isPrefill && focusedCard.householdName && (
                    <div className="font-sans text-[13px] text-cypress-100 mt-2">
                      {focusedCard.householdName}
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* BACK — form fields */}
              <div
                className="absolute inset-0 w-full rounded-2xl border-2 border-cypress-100 bg-white overflow-hidden"
                style={{
              ...
              >
                <div className="p-5 flex flex-col">
                  ...
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-2.5">
                      <div className="w-[32px] h-[32px] flex items-center justify-center">
                        {React.cloneElement(CARD_ICON[focusedCard.category] as React.ReactElement, { size: 32 })}
                      </div>
                      <span className="font-serif text-[16px] text-gray-100">
                        {focusedCard.label.replace("\n", " ")}
                      </span>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); closeCard(); }}
                      className="w-[26px] h-[26px] rounded-full bg-accent-subtle4x flex items-center justify-center text-gray-60 hover:text-gray-100"
                    >
                  ...
                  <TextInput
                    ...
                    onChange={(val) => updateCard(focusedCard.id, { name: val })}
                    readOnly={!!focusedCard.householdName && isPrefill}
                  />
                  <DateInput
                    label="Date of birth"
                    value={focusedCard.birthdate}
                    onChange={(val) => updateCard(focusedCard.id, { birthdate: val })}
                  />

                  <button
                    onClick={(e) => { e.stopPropagation(); saveCard(focusedCard.id); }}
                    className="mt-4 w-full py-3.5 rounded-lg bg-gray-100 text-white font-sans text-[16px] font-medium hover:opacity-90 active:opacity-80 transition-opacity disabled:opacity-40"
                    disabled={!focusedCard.name.trim()}
                  >
                    Save
                  </button>
                </div>
    ...
    return (
      <StepContent key="flipcard-main">
        ...
        <div className="space-y-4 mb-8">
          {/* Primary (large) card — centered */}
          {primaryCard && (
            <div className="flex justify-center">
              <div className="w-[60%]">
                {renderCardFront(primaryCard)}
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {secondaryCards.map((card) => renderCardFront(card))}
          </div>
        </div>
  ...
  return (
    <CanvasLayout
      ...
      currentPattern="flipcard"
      patternTitle="Flip Card Focus"
      patternDescription="Visual card layout with 3D flip animation. Tapping a card dims everything else and zooms into focus, creating a delight moment."
      patternBullets={[
        "Physical card metaphor with 3D flip animation",
        "Tapping a card dims everything else and zooms into focus",
        "\u201CBack of card\u201D holds the detail fields \u2014 creates a delight moment",
        "Visual hierarchy: primary beneficiary card is largest",
      ]}
    >
      <div className="relative" style={{ minHeight: "calc(100% - 55px)" }}>
        <EthosHeader onBack={handleBack} showBack={isComplete || focusedCardId !== null} />
