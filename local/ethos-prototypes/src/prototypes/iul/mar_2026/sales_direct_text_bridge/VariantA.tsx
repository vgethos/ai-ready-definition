 ...
 * Time slot cards let users commit to a call time before leaving.
 * Hypothesis: explicit opt-in commitment → higher connect rate.
 */
...
const TIME_SLOTS = [
  { id: 'today-2', label: 'Today', time: '2:00 PM' },
  { id: 'today-330', label: 'Today', time: '3:30 PM' },
...
export default function VariantA() {
  ...
  const selectedSlot = TIME_SLOTS.find(s => s.id === selected);

  return (
    <div className="min-h-screen" style={{ background: '#e5e5e7', padding: '24px 16px' }}>
      <VariantNav current="/iul/sales-direct-text-bridge/v1" />
      <PhoneFrame>
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '660px', background: 'white' }}>
          ...
          {confirmed ? (
            /* Confirmation state */
            <div style={{ flex: 1, padding: '40px 24px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'var(--cypress)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '24px' }}>
                <svg viewBox="0 0 24 24" width="28" height="28" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="20 6 9 17 4 12" />
                </svg>
              </div>
              <h1 style={{ ...type.h1, textAlign: 'center', marginBottom: '12px' }}>
                You're on the calendar.
              </h1>
              <p style={{ fontFamily: NH, fontSize: '16px', color: '#5d5d5d', textAlign: 'center', lineHeight: '26px' }}>
                Your IUL expert will call you {selectedSlot?.label.toLowerCase()} at {selectedSlot?.time} to go over your rate and next steps. Keep your phone nearby.
              </p>
            </div>
          ) : (
            /* Scheduling state */
            <div style={{ flex: 1, padding: '28px 24px 0', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <AgentAvatar />
              <h1 style={{ ...type.h1, textAlign: 'center', marginBottom: '12px' }}>
                You're pre-approved! When's a good time to go over your plan?
              </h1>
              <p style={{ ...type.cardDescription, textAlign: 'center', color: '#5d5d5d', lineHeight: '26px', marginBottom: '20px' }}>
                One of our IUL experts will walk you through your coverage and finalize your plan.
              </p>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        padding: '15px 18px',
                        borderRadius: '4px',
                        border: `1.5px solid ${isSelected ? 'var(--cypress)' : 'var(--night-20, #d4d4d4)'}`,
                        background: isSelected ? '#f0faf9' : 'white',
                        cursor: 'pointer',
                        textAlign: 'left' as const,
                        transition: 'border-color 0.15s, background 0.15s',
                      }}
                    >
                      <span style={{ fontFamily: NH, fontSize: '16px', fontWeight: 500, color: '#1a1a1a' }}>
                        {slot.label}
                      </span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ fontFamily: NH, fontSize: '16px', fontWeight: 500, color: isSelected ? 'var(--cypress)' : '#1a1a1a' }}>
                          {slot.time}
                        </span>
                        {isSelected && (
                          <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: 'var(--cypress)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <p style={{ fontFamily: NH, fontSize: '13px', color: '#9e9e9e', textAlign: 'center', marginTop: '12px' }}>
                Don't see a time that works? Check your texts.
              </p>
            </div>
          )}
          ...
          <div style={{ padding: '20px 24px 40px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {!confirmed && (
              <button
                ...
                style={{
                  width: '100%',
                  background: selected ? 'var(--cypress)' : 'var(--cypress)',
                  border: 'none',
                  borderRadius: '8px',
                  padding: '18px',
                  fontSize: '18px',
